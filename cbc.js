function ironProfile() {
  ...
  function wbcMain() {
    let wbcTotalVal = Number(labItems[0].value);
    let wbcMaxVal = Number(labItems[0].max);
    let wbcMinVal = Number(labItems[0].min);
    ...
    if (blastEntered == 1) {
      if (blast > 20) {
        ...
        try {
          signMaker(
            listMaker(
              arrayDuplicateRemover(["Acute Leukemia" , "Myeloproliferative Neoplasms" , "Myelodysplasia Syndrome" , "Reactive Source"]),
              "Peripheral Blast"
            ),
            path, //path
            0,

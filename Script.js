...
function    change_table(tab_id,test_cat)  {
    ...
    if(tab_id=='tab_analyse') {
        ...
        var illness_parent = document.createElement("div");
        illness_parent.className = "illness_parent";
        parent_element.appendChild(illness_parent);
        illness_parent.innerHTML = "Found illnesses: ";
        for(i=0; i<patient[0].illness.length; i++) {
            var illness_section = document.createElement("div");
            illness_section.className = "illness_section";
            illness_parent.appendChild(illness_section);
            
            try {
                illness_section.innerHTML += patient[0].illness[i];
            }
            catch{}
        }
    }
...
function    anemia()    {
    ...
    //to add new anemias 
    if (p_hb < mydata[2].min && p_mcv>0 && p_hb>0) {
        ...
        else if (p_mcv >= mydata[4].min && p_mcv<= mydata[4].max) {
            patient[0].signs.push("normocytic anemia");
        }
        else {}
    } else if (p_hb < mydata[2].min && p_hb>0) {
